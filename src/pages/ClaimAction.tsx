...
export default function ClaimAction() {
  ...
  const canProcessApprove = !processing && !!user && isAuthorizedReviewer && mode.isManager && isFinalSplitComplete;

  const processApprove = async () => {
    if (!claimId || !user || !isAuthorizedReviewer || !mode.isManager) return;
    setProcessing(true);
    try {
      await approveClaimAsManager(claimId, user.email, {
  ...
  const processReject = async () => {
    if (!claimId || !user || !isAuthorizedReviewer || !mode.isManager || !rejectReason.trim()) return;
    setProcessing(true);
    try {
      await rejectClaim(claimId, rejectReason.trim(), user.email, 'Manager');
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-8">
      <Card className="w-full max-w-6xl">
        <CardHeader className="space-y-1">
          <CardTitle>{mode.isManager ? 'Final Claim Review' : 'Claim Review'}</CardTitle>
        </CardHeader>
            ...
            <>
              ...
              {message ? (
                ...
                <div className="flex gap-2">
                  ...
                  {mode.isManager && (
                    <Button variant="destructive" onClick={() => void processReject()} disabled={processing || !user || !isAuthorizedReviewer || !mode.isManager || !rejectReason.trim()}>
                      {processing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                      Reject Claim
                    </Button>
                  )}
